...
/**
 * Base class for installers that install packages via pm shell command, child classes must provide a Shell{@link com.modosa.rootinstaller.utils.shell.Shell}
...
public abstract class ShellSAIPackageInstaller extends SAIPackageInstaller {
    ...
    private String getSessionInfo(ApkSource apkSource) {
        ...
        return String.format("%s: %s %s | %s | Android %s | Using %s ApkSource implementation | Install Lion-Root %s", getContext().getString(R.string.installer_device), Build.BRAND, Build.MODEL, Utils.isMiui() ? "MIUI" : "Not MIUI", Build.VERSION.RELEASE, apkSource.getClass().getSimpleName(), saiVersion);
    }
